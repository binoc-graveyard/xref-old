...
sub warning {
  my ($wmsg, $wclass) = @_;
  $wmsg = '' unless defined $wmsg;
  $wclass = '' unless defined $wclasss;
...
sub markupfile {
  ...
  if ($name =~ /\.(?:java|idl)$/i) {
    @terms = @javaterm;
  } elsif ($name =~ /\.(?:hh?|s|cpp?|c[cs]?|php?|mm?|pch\+?\+?|fin|tbl|ipdlh?)$/i) { # Duplicated in genxref.
    @terms = @cppterm;
  } elsif ($name =~ /\.(?:jsm?)(?:\.in|)$/) {
    @terms = @jsterm;
...
sub stylesheet {
  ...
  my $kind;
  $kind = 'style' if -f ('media/style/' . $pre . "style.css");
  my $ext;
  if ($Path->{'file'}) {
  ...
  $kind = $ext if -f ("media/style/$pre$ext.css");
  return '' unless $kind;
  return "$pre$kind.css";
}
...
sub stylesheets {
  ...
  my $stylesheets;
  while ($stylesheet) {
    $stylesheets .= "<link rel='$type' title='$title' href='/media/style/$stylesheet?20190929.0901' type='text/css'>\n" if $stylesheet;
